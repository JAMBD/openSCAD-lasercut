        ...
        #laserCutLayer(trans=[14.5-8,20/2-12,$thickness/2+5.15],rot=[0,0,90],thickness=$thickness,flatten = $flat){
            difference(){
                difference(){
                    ...
                    for(i=[0:1:4]){
                        translate([-$fingerWidth*2-(20/2-12)-i*2*$fingerWidth+$kerf/2,29/2-$thickness-0.25]){
                            square([$fingerWidth-$kerf,$thickness+0.5]);
                        }
                        translate([-$fingerWidth*2-(20/2-12)-i*2*$fingerWidth+$kerf/2,-29/2-0.25]){
                            square([$fingerWidth-$kerf,3.2+0.5]);
                        }
                    }
                }
                translate([-10,-29/2]){
        ...
        #laserCutLayer(trans=[-8+$thickness/2,-12/2-$clearance/2,5.15+25],rot=[90,0,90],thickness=$thickness,flatten  = $flat){
            difference(){
                difference(){
                    ...
                    union(){
                        for(i=[0:1:4]){
                            translate([$fingerWidth-(-12/2-$clearance/2)-i*$fingerWidth*2+$kerf/2,50/2-8-0.25]){
                                square([$fingerWidth-$kerf,8+0.5]);
                            }
                            translate([$fingerWidth-(-12/2-$clearance/2)-i*$fingerWidth*2+$kerf/2,-50/2-0.25]){
                                square([$fingerWidth-$kerf,$thickness+0.5]);
                            }
                        }
        ...
        #laserCutLayer(trans=[15-8,-12/2-$clearance/2,30],rot=[60,0,90],thickness=$thickness,flatten  = $flat){
            difference(){
                difference(){
                    ...
                    for(i=[0:1:4]){
                        translate([$fingerWidth-(-12/2-$clearance/2)-i*$fingerWidth*2+$kerf/2,-55/2-0.25]){
                            square([$fingerWidth-$kerf,3.5+0.5]);
                        }
                        translate([$fingerWidth*2-(-12/2-$clearance/2)-i*$fingerWidth*2+$kerf/2,55/2-7-0.25]){
                            square([$fingerWidth-$kerf,7+0.5]);
                        }
                    }
                }
...
assembly($num-10,$flat){
    ...
    for(ang=[0:120:359]){
        rotate([0,0,ang]){
            laserCutLayer(trans=[0,-19,($leverHeight+$thickness*3+$servoHeight)/2-$thickness*1.5],rot=[90,90,0],thickness=$thickness,flatten  = $flat){
                difference(){
                    union(){
                        square([$leverHeight+$thickness*3+$servoHeight,15],center=true);
                        translate([($leverHeight+$thickness*2+$servoHeight)/2-3-$thickness*2.5,0]){
                            square([6,20],center=true);
                        }
                        
                        translate([-($leverHeight+$thickness*2+$servoHeight)/2+3+$thickness*0.5,0]){
                            square([6,20],center=true);
                        }
                    }
    ...
    laserCutLayer(trans=[0,0,$leverHeight+$thickness+$servoHeight],rot=[0,0,60],thickness=$thickness,flatten  = $flat){
        difference(){
    ...
    #laserCutLayer(trans=[0,0,$leverHeight+$thickness],rot=[0,0,0],thickness=$thickness,flatten  = $flat){
        difference(){
            union(){
                for(ang=[0:120:359]){
                    rotate([0,0,ang]){
    ...
    laserCutLayer(trans=[0,0,$leverHeight],rot=[0,0,0],thickness=$thickness,flatten  = $flat){
        difference(){
            union(){
                for(ang=[0:120:359]){
                    rotate([0,0,ang]){
